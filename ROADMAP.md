...
- [x] **模型配置**：批量删除（含默认模型警示）、Esc 关闭弹窗、名称重复校验、Max Tokens 校验、分组/折叠视图持久化、测试结果带时间戳、组头健康计数（失败 / 未测试，折叠后仍可见）与一键展开或折叠全部分组；分组视图改为**纵向面板**：每组一块面板、组内模型为全宽行（信息左、操作右对齐），消除并排分组的高度参差；视口 ≥1000px 时分组按估算高度均衡分到两列（`matchMedia` + 贪心分配，折叠状态变化会重排），窄窗口回退单列
