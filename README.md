...
- **Harness**：启动/停止本地 `@deepseek-ai/dsh`，iframe 内嵌 Web UI；侧栏切走后保活；自定义启动端口；浏览器打开；空态内嵌「安装」按钮；可选进入页面自动启动（记忆开关）；运行中每 8 秒探测进程，外部关闭服务后状态自动回落
- **快问**：托盘常驻；默认 `Ctrl+Alt+K`；桌面浮漂；流式问答；多轮追问 / 重新生成；可选工作区 / Git / Dirty / 剪贴板上下文
- **模型配置**：多厂商与 OpenAI 兼容接口；增删改查、连接测试、批量测试/同步/删除、分组视图（按提供商分成纵向面板，组头显示失败 / 未测试计数并可一键展开或折叠全部；扁平与分组共用同一卡片网格 —— `auto-fill` 等宽等高，组内模型横向并排，单模型分组保留等宽空位，信息行不换行、超长自动省略）、同步 DSH；批量测试并发 4 路并显示进度，测试结果（通过/失败 + 时间）落库、重启后仍在；支持按测试状态筛选与排序；随机度 / 最长输出改为档位化选择（1K→1M 对数滑杆）
- **AI 提示词**：基于已配置模型改写提示词
...
- **WorkBuddy**：腾讯 CodeBuddy 账号池管理控制台 + **OpenAI 兼容反代网关**。扫码/粘贴纳管多个账号（凭证本机明文入库，界面只显示掩码）、每日自动签到（串行 + 账号间隔 2 秒防风控，连败 5 次自动停用）、分发网关密钥（`wk-` 前缀，明文仅创建/重置时出现一次）、IP 白/黑名单、调用日志与按天/模型/密钥的用量统计。网关默认只监听 `127.0.0.1`，局域网访问是显式开关且无密钥时拒绝开启；上游登录页 / 签到 / 对话端点全部走可编辑的协议 JSON（`wb_settings.adapter_json`），改协议不需要重新构建；协议面板顶部的**接口探针**会在登录窗里记录页面自己发出的接口调用（只记方法 / 路径 / 状态码 / 认证头名，不记任何值），点一下即可写入对应端点配置
...
- **Git 报告**：左侧仓库 / 工作区树 + 右侧五视图报告（概览、改动文件、提交、API、AI）；改动基准可选未提交改动 / 相对基准 / 最近 N 个提交 / 指定日期区间（两端含当日）；按 Git diff 静态推导模块 / 层级 / 风险 / 缺测 / 接口变更，选中工作区文件夹可把同夹多仓合并成一份报告；支持文件路径搜索、层级 / 风险筛选、高风险摘要一键定位、仓库模式记忆；AI 可粘贴需求 / 验收标准，据此追加「需求覆盖对照」并把「必测场景」输出为 P0/P1/P2 用例卡片（优先级 / 前置 / 步骤 / 预期），整份报告含需求原文可导出 Markdown；AI 结论生成成功即留存本机历史（按目标 / 基准 / 模型 / 需求快照归档，最多 50 条）可回看与复制。报告本体不落库、不执行测试
...
- **小工具**：10 个开发常用工具，按「转换 / 生成 / 解析 / 网络」分组——JSON（格式化 / 压缩 / 修复 / JSONPath / 历史）、编解码（Base64 / Base64URL / URL）、时间戳（秒 / 毫秒 / 微秒互转 + 实时时钟）、Cron（解析 + 自然语言 + 下次执行 + 模板）、UUID（批量生成 + 校验）、Hash（SHA-1/256/384/512 + 对比）、JWT（解码 + 过期状态）、正则（实时匹配 + 分组 + 替换）、端口 / 进程（netstat + 进程详情 + 结束进程 + 自动刷新）、HTTP 客户端（多方法 / 参数 / 请求头 / 响应格式化 / 历史 / 收藏 / cURL 导入导出）。侧栏支持搜索过滤与「最近使用」；`1-9` 数字键快速切换；页面保活，切走再回来输入不丢
...
- **快捷应用**：保存常用应用路径（名称 / exe / 启动参数 / 分组 / 版本），一键启动与关闭。「添加应用」直接打开扫描弹窗（注册表 + 安装目录 + 开始菜单/桌面快捷方式，直接解析 .lnk 二进制，覆盖其他盘符与深层布局，并对 Codex / Claude Code 等无注册表、无快捷方式的 per-user CLI 工具走一份可扩充的精选清单，自动识别主程序、读取程序版本、带真实图标）批量导入，弹窗底部「手动添加…」唤起表单；运行时状态检测（tasklist 批量查询 + 可选 5 秒自动刷新）、单个/批量关闭（先礼后兵：正常退出超时再强杀）；拖拽排序 + 排序方式（自定义 / 名称 / 分组 / 运行中优先）、自定义分组（分组管理器新建/重命名/删除，重命名与删除会同步到应用；分组列表存 localStorage `al-groups`，归属仍在应用的 `group` 字段入库；筛选下拉展示空分组，表单可选/建分组，多选可批量设置分组）、搜索、多选批量启动/删除；应用版本自动读取（Win32 文件版本资源，卡片上直接展示，可手动覆盖）；JSON 导入导出（按路径去重）；原生文件选择器浏览 exe
...
- 启动时自动检测新版本（Gitee 优先、GitHub 兜底，均不可达则静默），右下角非阻塞提示条，可「查看更新」跳转发布页或「跳过此版本」（记入 localStorage `update-skip-version`，发新版后重新提醒）；设置页也可手动检查
...
- API Key、Tunnel Token、Cursor 密码备忘、CodeBuddy 账号凭证、网关分发密钥等保存在本机 SQLite：`%APPDATA%\com.ai-workbench.app\ai-workbench.db`
- 不会上传密钥；请勿分享数据库或导出的含密钥 JSON
- WorkBuddy 的 OpenAI 兼容网关默认只监听 `127.0.0.1`；「允许局域网访问」是显式开关，开启后同网段设备可凭分发密钥使用你的账号池，请同时配好 IP 白名单（本程序不会自动修改防火墙规则，只提供可复制的 netsh 命令）
