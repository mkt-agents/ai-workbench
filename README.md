...
- **快捷应用**：保存常用应用路径（名称 / exe / 启动参数 / 分组 / 版本），一键启动与关闭。支持扫描本机已安装应用（注册表 + 安装目录 + 开始菜单快捷方式，直接解析 .lnk 二进制，覆盖其他盘符与深层布局，自动识别主程序、带真实图标）批量导入；运行时状态检测（tasklist 批量查询 + 可选 5 秒自动刷新）、单个/批量/全部关闭（先礼后兵：正常退出超时再强杀）；拖拽排序 + 排序方式（自定义 / 名称 / 分组 / 运行中优先）、分组筛选、搜索、多选批量启动/删除；应用版本自动读取（Win32 文件版本资源，卡片上直接展示，可手动覆盖）；JSON 导入导出（按路径去重）；原生文件选择器浏览 exe
