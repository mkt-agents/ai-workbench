...
- **Git 报告**：左侧仓库 / 工作区树 + 右侧五视图报告（概览、改动文件、提交、API、AI）；按 Git diff 静态推导模块 / 层级 / 风险 / 缺测 / 接口变更，选中工作区文件夹可把同夹多仓合并成一份报告；支持文件路径搜索、层级 / 风险筛选、高风险摘要一键定位、仓库模式记忆；AI 可粘贴需求 / 验收标准，据此追加「需求覆盖对照」并把「必测场景」输出为 P0/P1/P2 用例卡片（优先级 / 前置 / 步骤 / 预期），整份报告含需求原文可导出 Markdown。报告不落库、不执行测试
